...
impl ToTokens for ComposeInput {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        ...
        let mm = vec![3, 4, 55];
        let res = mm.iter().rfold(3.4, |acc, e| acc + *e as f64 );

        as_idents.iter().rfold(quote!(#last_ident), |acc, ident| {

            quote!(
                compose_two(#ident, #acc)
            )
        } ).to_tokens(tokens);
...
pub fn compose_impl(item: TokenStream) -> TokenStream {
